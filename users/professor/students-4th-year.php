...
$professor_id = $_SESSION['user_id'];
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['action']) && $_POST['action'] == "add_or_edit") {
            ...
            if (!$student_id || !$subject_id || !$professor_id || $grade === null) {
            ...
            if ($count > 0) {
                // Update grade, subject, and professor
                $stmt = $con->prepare("UPDATE first_semester_students 
                                    SET grade = ?, subject_id = ?, professor_id = ?, date_added = ? 
                                    WHERE student_id = ?");
                $stmt->bind_param("siiis", $grade, $subject_id, $professor_id, $current_time, $student_id);
                ...
                // Insert new grade
                $stmt = $con->prepare("INSERT INTO first_semester_students (student_id, grade, date_added, professor_id, subject_id) 
                                    VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("issii", $student_id, $grade, $current_time, $professor_id, $subject_id);
                $success = $stmt->execute();
                $stmt->close();
