                        ...
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            if (isset($_POST['action']) && $_POST['action'] == "add_or_edit") {
                                ...
                                $stmt = $con->prepare("SELECT full_name FROM fourth_year WHERE id = ?");
                        ...
                        // Fetch students with grades from fourth_year table
                        $students_query = mysqli_query($con, "
SELECT fy.id AS student_id, fy.full_name, fy.profile_image, fy.created_at, 
       fs.grade, fs.subject_id, fs.professor_id, 
       s.subject_name, p.full_name AS professor_name
FROM fourth_year fy
LEFT JOIN first_semester_students fs ON fy.id = fs.student_id
