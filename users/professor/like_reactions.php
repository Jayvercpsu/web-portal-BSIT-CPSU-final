                ...
                $result = mysqli_query($con, "SELECT role FROM users WHERE id = '$user_id' AND role = 'professor'");
                $professor = mysqli_fetch_assoc($result);

                if (!$professor) {
                    echo json_encode(['status' => 'error', 'message' => 'Only professors can react']);
                    exit();
                }
                ...
                if (mysqli_num_rows($query) > 0) {
                    ...
                    // Decrement the like count in professors_post table
                    $update_post = mysqli_query($con, "UPDATE professors_post SET likes = likes - 1 WHERE id = '$post_id' AND likes > 0");
                    ...
                    // Increment the like count in professors_post table
                    $update_post = mysqli_query($con, "UPDATE professors_post SET likes = likes + 1 WHERE id = '$post_id'");
