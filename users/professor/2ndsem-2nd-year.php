...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action']) && $_POST['action'] == "add_or_edit") {
        ...
        $stmt = $con->prepare("SELECT COUNT(*) FROM second_semester_students WHERE student_id = ? AND subject_id = ?");
        ...
        if ($count > 0) {
            ...
            $stmt = $con->prepare("UPDATE second_semester_students 
            ...
            // Insert new record
            $stmt = $con->prepare("INSERT INTO second_semester_students (student_id, full_name, grade, date_added, professor_id, subject_id) 
                                                            VALUES (?, ?, ?, ?, ?, ?)");
    ...
    if (isset($_POST['action']) && $_POST['action'] == "delete") {
        ...
        if ($student_id > 0) {
            $stmt = $con->prepare("DELETE FROM second_semester_students WHERE student_id = ?");
            $stmt->bind_param("i", $student_id);
            $success = $stmt->execute();
            $stmt->close();
            $response = [
            ...
            echo "<script>
                                            ...
                                            window.location.href = '2ndsem-2nd-year.php';
                                        </script>";
        } else {
...
FROM users u
LEFT JOIN second_semester_students fs ON u.id = fs.student_id
...
ORDER BY u.id ASC
");
$second_sem_students = mysqli_fetch_all($students_query, MYSQLI_ASSOC);
    ...
    <div class="content">
        <div class="container">
            ...
            <table id="secondSemesterTable" class="table table-bordered">
                ...
                <tbody>
                    <?php foreach ($second_sem_students as $student) { ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                <?php if (!empty($student['grade'])) { ?>
                                    ...
                                    <form method="POST" action="2ndsem-2nd-year.php" style="display:inline;">
