<div id="grade-form-2nd">
    <h3 class="text-center">SECOND YEAR GRADE</h3>

    <form method="post" id="grade-entry-form-2nd">
        ...
        <input type="hidden" name="year_form[]" value="2">
        ...
        <table class="table table-bordered">
            ...
            <tbody id="grade-table-body-2nd-sem-1st"></tbody>
        </table>
        ...
        <table class="table table-bordered">
            ...
            <tbody id="grade-table-body-2nd-sem-2nd"></tbody>
        </table>

        <button type="submit" class="btn btn-success mt-3">Submit Grades</button>
        <p id="success-message-2nd" class="text-success mt-2" style="display:none;">Grades successfully saved!</p>
    </form>
</div>
...
<script>
    $(document).ready(function() {
        ...
        function loadCourses() {
            ...
            $("#selected_student_id_2nd").val(studentId);
            $("#selected_student_name_2nd").val($("#student_id option:selected").text());

            let tbody1st = $("#grade-table-body-2nd-sem-1st");
            let tbody2nd = $("#grade-table-body-2nd-sem-2nd");
            tbody1st.html("");
            tbody2nd.html("");

            courses["1st Sem"].forEach((course, index) => {
                tbody1st.append(createTableRow(course, `2-1-${index}`, "1st Sem"));
            });

            courses["2nd Sem"].forEach((course, index) => {
                tbody2nd.append(createTableRow(course, `2-2-${index}`, "2nd Sem"));
            });
        ...
        $("#grade-entry-form-2nd").on("submit", function(e) {
            e.preventDefault();
            let studentId = getStudentId();
            if (!studentId) {
